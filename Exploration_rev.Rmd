...
2) calcolare alcune misure statistische relative ai gruppi formati in fase di discretizzazione e scegliere la misura per settare la soglia (lower & upper);  
3) individuare il lower e l'upper bound sulla base dell'indice scelto.  

Per discretizzare la variabile "giocate totali", abbiamo considerato i quartili della distribuzione di frequenza. Fatta questa premessa, sarebbe stato un errore utilizzare la distribuzione di tale variabile cos? com'?: come si evince dall'output precedente, in pi? di un mese, i valori delle giocate totali nei due Casin? sono significativamente diversi. Per non incorrere nell'errore di creare intervalli errati si ? deciso che per ciascun mese la variabile "giocate totali" verr? dicretizzata con intervalli differenti a seconda del Casin? di appartenenza.
