...
--> quindi ci aspettiamo molte piu' video che reel in Aries. --> possibile vincolo sul tipo (video, reel), chiediamo al Manager?!  
Concludiamo il discorso dicendo che, come nel caso precedente, è possibile valurare valore e regione di validità dei prezzi ombra per individuare possibili variazioni nel numero di slot per sezione.
...
Data l'elevato numero di vincoli e quindi la complessità generata in fase di valutazione ci limiteremo a valutare quali categorie hanno ricevuto il numero di slot massimo.
```{r}
knitr::kable(vincolo3[vincolo3$actual == vincolo3$bvec & vincolo3$dir == "<=", ])
```
Come si può facilemente notare le categorie più nobili, in termini di numero di slot ricevute, sono quelle che nel grafico X (file Exploration) si distinguevano per una propensione elevata al ricavo unitario: reel and video per entrmabi i Casinò; Sono proprio queste due categorie a prevalere nella precedente tabella segno che le stesse portano a ricavi maggiori. 
...
```
Si nota che la distribuzione di tali valori è molto distribuita quindi anche variazioni considerevoli di un valore dei coefficienti della F.O., non cambierebbe la soluzione ottima trovata.