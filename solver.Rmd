...
Rimane un unico mistero che non riesco a capire: perchè se lancio "as.tibble(df) %>% slice(rep(1:n(), each = 2)) %>% as.matrix() " il nuovo dataset raddoppiato è ordinato per casinò? mentre la matrice che raddoppio (f.A3) mantiene lo stesso ordinamento?
...
OKAY GABRI HO FILTRATO PER I DIVERSI MESI E C'E' UNA BUONA ED UNA CATTIVA NOTIZIA:  
- la buona è che il codice funziona per tutti i mesi quindi con quindi con un ciclo FOR che itera sui 12 mesi penso che possiamo cavarcela facilmente. Possiamo salvare i risultati in una lista o vedi se ti viene un'idea migliore. Magari nella lista ci conviene salvare anche i ricavi totali del mese per il quale si è MAX la funzione per poi fare le varie comparazioni;  
- la brutta è che per 3 mesi la soluzione non è "feasible". I mesi in questione sono: marzo, aprile e luglio. I solver che hai implementato tu non restituisce una soluzione mentre quello che ho messo io sì, ma è chiaramente tarocca. Quindi il tuo codice era giusto, ma c'erano mesi dove la soluzione non poteva essere trovata. 
...
```
Observations:  
- Each constraint is binding: the total number of machines for each Casinò is equal to the previous configuration;  
...
Conclusion: If we have to increase the number of machines for only one Casinò, we'll choose Aries
